...
let FixtureSyncService = FixtureSyncService_1 = class FixtureSyncService {
    ...
    async getOrCreateLeague(apiFixture) {
    ...
    extractCountryCodeFromFlag(flagUrl) {
        const match = flagUrl.match(/\/([a-z]{2})\.svg$/i);
        return match ? match[1].toUpperCase() : null;
    }
    async syncFixturesByDate(dateFrom, dateTo, onProgress) {
        const result = {
            totalFetched: 0,
            created: 0,
            updated: 0,
    ...
    async syncFixturesBatch(fixtures, leagueIdMap) {
        const result = { created: 0, updated: 0, skipped: 0, errors: [], autoCreatedLeagues: 0, autoCreatedTeams: 0 };
        if (fixtures.length === 0)
            return result;
        const now = new Date();
        const minDate = new Date(now);
        minDate.setDate(minDate.getDate() - FixtureSyncService_1.MAX_PAST_DAYS);
        const maxDate = new Date(now);
        maxDate.setDate(maxDate.getDate() + FixtureSyncService_1.MAX_FUTURE_DAYS);
        ...
        if (toUpdate.length > 0) {
            for (let i = 0; i < toUpdate.length; i += FIXTURE_BATCH_SIZE) {
                ...
                await this.prisma.$transaction(batch.map(({ id, data }) => this.prisma.match.update({ where: { id }, data })));
            }
    ...
    async syncSingleFixture(apiFixture) {
        ...
        if (fixtureDate < minDate || fixtureDate > maxDate) {
            this.logger.debug(`Skipping fixture ${fixtureExternalId} outside sync window (${fixtureDate.toISOString()})`);
            return { skipped: true };
        }
...
FixtureSyncService.MAX_PAST_DAYS = 2;
FixtureSyncService.MAX_FUTURE_DAYS = 14;
