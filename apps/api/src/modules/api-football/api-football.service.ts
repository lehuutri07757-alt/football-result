...
export class ApiFootballService implements OnModuleInit {
  ...
  private async loadProviderConfig(): Promise<void> {
    try {
      ...
      const apiKey = this.configService.get<string>('API_FOOTBALL_KEY') || provider.apiKey || '';
  ...
  async getFixtureOdds(fixtureId: number): Promise<OddsTableRow | null> {
    ...
    const isLive = LIVE_STATUSES.includes(fixture.fixture.status.short as FixtureStatusShort);
  ...
  async getApiLogsStats(days = 7) {
    ...
    const [totalRequests, successCount, errorCount, avgResponseTime, requestsByEndpoint, requestsByDay] =
      await Promise.all([
    ...
    return {
      ...
      requestsByEndpoint: requestsByEndpoint.map((r: { endpoint: string; _count: { id: number } }) => ({
        endpoint: r.endpoint,
        count: r._count.id,
      })),
      requestsByDay,
  ...
  private async fetchPreMatchOdds(fixtureId: number): Promise<OddsMarket[]> {
    ...
    const targetBookmaker = bookmakers.find((b) => b.id === DEFAULT_BOOKMAKER_ID) || bookmakers[0];
  ...
  private async fetchLiveOdds(fixtureId: number): Promise<ApiLiveOddsResponse | null> {
    const response = await this.makeApiRequest<ApiLiveOddsResponse>('/odds/live', {
      fixture: fixtureId.toString(),
    });
  ...
  private async makeApiRequest<T>(
    ...
    if (cacheKey) {
      this.inFlightRequests.set(cacheKey, requestPromise as Promise<ApiFootballResponse<unknown>>);
    }
  ...
  private buildCacheKey(endpoint: string, params: Record<string, string>): string {
    ...
    const hash = createHash('sha256').update(`${endpoint}?${canonicalParams}`).digest('hex').slice(0, 32);
    return `${this.apiCachePrefix}:${endpoint.replaceAll('/', '_')}:${hash}`;
  }

  private getCacheTtlSeconds(endpoint: string, params: Record<string, string>): number {
  ...
  private sanitizeHeadersForLogging(headers: Record<string, string>): Record<string, string> {
  ...
  private transformFixtureToRow(fixture: ApiFixture, odds: OddsMarket[]): OddsTableRow {
    const isLive = LIVE_STATUSES.includes(fixture.fixture.status.short as FixtureStatusShort);
    const matchTime = isLive
    ...
    return {
      ...
      homeGoalOU: this.extractTeamTotal(odds, API_FOOTBALL_BET_IDS.HOME_TEAM_TOTAL),
      awayGoalOU: this.extractTeamTotal(odds, API_FOOTBALL_BET_IDS.AWAY_TEAM_TOTAL),
      btts: this.extractBTTS(odds),

      htHdp: isLive ? this.extractAsianHandicap(odds, API_FOOTBALL_BET_IDS.HT_ASIAN_HANDICAP) : undefined,
      htOverUnder: isLive ? this.extractOverUnder(odds, API_FOOTBALL_BET_IDS.HT_OVER_UNDER) : undefined,
      htOneXTwo: isLive ? this.extractMatchWinner(odds, API_FOOTBALL_BET_IDS.HT_MATCH_WINNER) : undefined,

      totalMarkets: odds.length,
    };
  }

  private extractAsianHandicap(odds: OddsMarket[], betId: number): MatchOdds | undefined {
  ...
  private extractOverUnder(odds: OddsMarket[], betId: number): MatchOdds | undefined {
    const market = odds.find((o) => o.id === betId);
    if (!market) return undefined;
  ...
  private extractMatchWinner(odds: OddsMarket[], betId: number): MatchOdds | undefined {
    const market = odds.find((o) => o.id === betId);
    if (!market) return undefined;
  ...
  private extractTeamTotal(odds: OddsMarket[], betId: number): MatchOdds | undefined {
    const market = odds.find((o) => o.id === betId);
    if (!market) return undefined;
  ...
  private extractBTTS(odds: OddsMarket[]): MatchOdds | undefined {
    const market = odds.find((o) => o.id === API_FOOTBALL_BET_IDS.BOTH_TEAMS_SCORE);
    if (!market) return undefined;
  ...
  private readonly INTERNATIONAL_KEYWORDS = ['World', 'UEFA', 'CONMEBOL', 'AFC', 'CAF', 'CONCACAF', 'OFC', 'International', 'Euro'];
  ...
  private async fetchUpcomingFixtures(date: string): Promise<ApiFixture[]> {
    const response = await this.makeApiRequest<ApiFixture>('/fixtures', { date });
    return response.response;
  }
  ...
  private getInternationalsStats(fixtures: ApiFixture[]): CountryLeagueStats {
    const internationalFixtures = fixtures.filter(f =>
      this.INTERNATIONAL_KEYWORDS.some(keyword => 
        f.league.name.includes(keyword) || f.league.country.includes(keyword) || f.league.country === 'World'
      )
    );

    const leagueMap = new Map<number, { id: number; name: string; logo: string; matchCount: number }>();
  ...
  private calculateTopLeaguesStats(countries: CountryLeagueStats[]): CountryLeagueStats {
    const totalMatches = countries.reduce((sum, c) => sum + c.matchCount, 0);
    const allLeagues: Array<{ id: number; name: string; logo: string; matchCount: number }> = [];
  ...
  async fetchAllLeagues(onlyCurrentSeason = true): Promise<ApiLeagueInfo[]> {
    ...
    const response = await this.makeApiRequest<ApiLeagueInfo>('/leagues', params);
    return response.response;
  }
