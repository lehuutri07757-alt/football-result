...
export function DesktopBetSlip() {
  ...
  return (
    <aside className="hidden lg:flex w-80 flex-col border-l border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 sticky top-0 h-[calc(100vh-64px)] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-emerald-500 text-white flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          {itemCount > 0 && (
            <span className="min-w-[20px] h-5 px-1.5 flex items-center justify-center text-[10px] font-bold bg-white/25 rounded-full">
              {itemCount}
            </span>
          )}
        </div>
              ...
              {lastPlacedBet.selections.map((sel) => (
                <div key={sel.id}>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {sel.selectionName || sel.selection} @ {Number(sel.oddsValue).toFixed(2)}
                  </p>
                </div>
              ))}
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 pt-2 mt-2 space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900 dark:text-white">
                      {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(lastPlacedBet.stake)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 dark:text-slate-400">Potential Win</span>
                    <span className="font-bold text-emerald-600 dark:text-emerald-400">
                      {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(lastPlacedBet.potentialWin)}
                    </span>
                  </div>
              </div>
            </div>
      ...
      {itemCount > 0 && !showConfirmation && (
        <div className="border-t border-slate-200 dark:border-slate-700 p-4 space-y-3 flex-shrink-0 bg-white dark:bg-slate-900">
