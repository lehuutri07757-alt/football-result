      ...
      <div className="flex items-center justify-between border-b px-4 py-3 text-xs text-muted-foreground">
        <div className="flex items-center gap-2">
          {match.league?.logoUrl ? (
            <img src={match.league.logoUrl} alt={match.league.name} className="h-4 w-4 object-contain" />
          ) : (
            <Trophy className="h-3.5 w-3.5" />
          )}
          <span className="font-medium truncate max-w-[150px]">{match.league?.name || 'Unknown League'}</span>
        </div>
          ...
          <div className="flex flex-1 flex-col items-center gap-3 text-center">
            <div className="relative h-12 w-12 flex items-center justify-center rounded-full bg-muted/50 p-2 group-hover:bg-emerald-500/10 transition-colors">
              {match.homeTeam?.logoUrl ? (
                <img src={match.homeTeam.logoUrl} alt={match.homeTeam.name} className="h-full w-full object-contain" />
              ) : (
                <div className="text-xs font-bold">{match.homeTeam?.name?.substring(0, 2).toUpperCase()}</div>
              )}
            </div>
            <span className="text-sm font-semibold leading-tight line-clamp-2">{match.homeTeam?.name || 'Home Team'}</span>
          </div>

          <div className="flex flex-col items-center gap-1 min-w-[80px]">
            {isLive || isFinished ? (
              <div className="flex items-center gap-3 text-2xl font-bold font-mono tracking-tight">
                <span className={cn(match.homeScore! > match.awayScore! ? "text-emerald-600 dark:text-emerald-400" : "")}>
                  {match.homeScore ?? 0}
                </span>
                <span className="text-muted-foreground/50">-</span>
                <span className={cn(match.awayScore! > match.homeScore! ? "text-emerald-600 dark:text-emerald-400" : "")}>
                  {match.awayScore ?? 0}
                </span>
              </div>
            ) : (
              <div className="flex flex-col items-center text-muted-foreground group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                <span className="text-xl font-bold tracking-tight">{formatTime(match.startTime)}</span>
                <span className="text-[10px] font-medium uppercase">{formatDate(match.startTime)}</span>
              </div>
            )}
          ...
          <div className="flex flex-1 flex-col items-center gap-3 text-center">
            <div className="relative h-12 w-12 flex items-center justify-center rounded-full bg-muted/50 p-2 group-hover:bg-emerald-500/10 transition-colors">
              {match.awayTeam?.logoUrl ? (
                <img src={match.awayTeam.logoUrl} alt={match.awayTeam.name} className="h-full w-full object-contain" />
              ) : (
                <div className="text-xs font-bold">{match.awayTeam?.name?.substring(0, 2).toUpperCase()}</div>
              )}
            </div>
            <span className="text-sm font-semibold leading-tight line-clamp-2">{match.awayTeam?.name || 'Away Team'}</span>
          </div>
        </div>
      </Link>
      ...
      {match.bettingEnabled && !['finished', 'cancelled', 'postponed'].includes(match.status) && (
        <div className="grid grid-cols-3 gap-px border-t bg-border/50">
          <button className="flex flex-col items-center justify-center bg-card py-2.5 hover:bg-emerald-50 hover:text-emerald-700 dark:hover:bg-emerald-950/30 dark:hover:text-emerald-400 transition-colors">
            ...
            <span className="text-sm font-bold">{(Math.random() * 2 + 1.5).toFixed(2)}</span>
          </button>
          <button className="flex flex-col items-center justify-center bg-card py-2.5 hover:bg-emerald-50 hover:text-emerald-700 dark:hover:bg-emerald-950/30 dark:hover:text-emerald-400 transition-colors">
            <span className="text-[10px] text-muted-foreground font-medium mb-0.5">X</span>
            <span className="text-sm font-bold">{(Math.random() * 2 + 2.5).toFixed(2)}</span>
          </button>
          <button className="flex flex-col items-center justify-center bg-card py-2.5 hover:bg-emerald-50 hover:text-emerald-700 dark:hover:bg-emerald-950/30 dark:hover:text-emerald-400 transition-colors">
            <span className="text-[10px] text-muted-foreground font-medium mb-0.5">2</span>
            <span className="text-sm font-bold">{(Math.random() * 2 + 1.5).toFixed(2)}</span>
          </button>
        </div>
      )}
